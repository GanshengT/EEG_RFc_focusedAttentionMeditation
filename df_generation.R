...
df_generation=function(storyFilename,MdtFilename){
  setwd("E:/important file/CENTRALE/BCI ET EEG/Rprocess/data")
  #filename=readline(prompt = "Enter the data file name")
  HU=read.table(file=paste0(storyFilename,".txt"),header = T,sep = ",",dec = ".",skip = 11,fill = T)
  #keep the same size
  ...
  HUm=read.table(file=paste0(MdtFilename,".txt"),header = T,sep = ",",dec = ".",skip = 11,fill = T)
  ...
  export_df_name=paste(storyFilename,MdtFilename,"_df.csv")
