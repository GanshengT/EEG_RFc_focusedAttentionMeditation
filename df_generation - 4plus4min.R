...
df_generation=function(MdtFilename,StoryFilename){
  ...
  HUm=read.table(file=paste0(StoryFilename,".txt"),header = T,sep = ",",dec = ".",skip = 11,fill = T)
  HUm=HUm[((nrow(HUm)-242*256)/2+1):((nrow(HUm)-242*256)/2+242*256),]
  ...
  df1=data.frame(time=seq(241,240+240))
  HR=approx(x,HUm$Heart.Rate,n=480)$y
  HR=HR[-1]
  HR=HR[-length(HR)]
  x=seq(length(HR))
  df1$HR=approx(x,HR,n=240)$y
  
  #interpolation of the SC
  x=seq(length(HUm$SC))
  #df=data.frame(time=seq(450))
  SC=approx(x,HUm$SC,n=480)$y
  SC=SC[-1]
  SC=SC[-length(SC)]
  x=seq(length(SC))
  df1$SC=approx(x,SC,n=240)$y
  
  #interpolation of the St
  x=seq(length(HUm$Temp))
  #df=data.frame(time=seq(450))
  ST=approx(x,HUm$Temp,n=580)$y
  ST=ST[-1]
  ST=ST[-length(ST)]
  x=seq(length(ST))
  df1$ST=approx(x,ST,n=240)$y
  
  #interpolation of the Respiration
  x=seq(length(HUm$Respiration.Rate))
  #df=data.frame(time=seq(450))
  Resp=approx(x,HUm$Respiration.Rate,n=580)$y
  Resp=Resp[-1]
  Resp=Resp[-length(Resp)]
  x=seq(length(Resp))
  df1$Resp=approx(x,Resp,n=240)$y
  ...
  export_df_name=paste(MdtFilename,StoryFilename,"4plus4min_df.csv")
