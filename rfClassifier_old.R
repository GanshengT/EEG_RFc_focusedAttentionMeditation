...
plot(df$SC,df$HRV,type = "n",ylim=c(100,300),main = "detailed tendency of Heart Rate Variability against Skin Conductance",xlab = "skin conductance(micro Siemens)",ylab="HRV(square of number of heart rate(beat per minute)")
text(df$SC,df$HRV,rownames(df),col=c("blue","red")[df$MediOrStory],cex=0.75)
library(e1071)
...
RfAccuracy_reExp=function(subjectName){
  ...
  #df=read.csv(file="MOHUstoryBio MOHUmeditationBio _df.csv")
  df$MediOrStory<-factor(ifelse(df$MediOrStory%in%c('0'),'0','1')) 
  df_test$MediOrStory<-factor(ifelse(df$MediOrStory%in%c('0'),'0','1')) 
  library(dplyr)
  library(e1071)
  library(tidyverse)
  library(caret)
