...
df=read.csv(file="hugues_auto/hugues_df.csv")
...
#we will obtain a pval table here
pValTable=data.frame("Model"="ini","pVal of HU"=0)
...
#conclusion HRV C ST is can be done a linear regression, notHR and Resp
# 
# df_bio$MediOrStory=df$MediOrStory
# dfslted=df_bio[which(df_bio[,'MediOrStory']==1),]
# dfslted=dfslted[which(dfslted[,'HRV']<ybounds(dfslted[,'HRV'])[2]),]
# dfslted=dfslted[which(dfslted[,'HRV']>ybounds(dfslted[,'HRV'])[1]),]
# dfslted=dfslted[which(dfslted[,'SC']<ybounds(dfslted[,'SC'])[2]),]
# dfslted=dfslted[which(dfslted[,'SC']>ybounds(dfslted[,'SC'])[1]),]
...
# pValTable=data.frame("Model"="ini","pVal of HU"=0)
# pValTable$Model=as.character(pValTable$Model)
# f <- summary(regSC_HRV_Medi)$fstatistic
# p <- pf(f[1],f[2],f[3],lower.tail=F)
# attributes(p) <- NULL
# pValTable=rbind(pValTable,c(as.character("SCHRVMedi"),p))
# 
...
#do the same between HRV and EEG signals
#after reading csv must change to factor
df$MediOrStory<-factor(ifelse(df$MediOrStory%in%c('0'),'0','1'))
for (col in 1:(ncol(df_eeg))){
  ...
  #plot(df_eeg[,col],df_eeg[,'HRV'],ylab="HRV",xlab=colnames(df_eeg[col]),col=c("red","blue")[df_eeg$MediOrStory])
      ...
      attributes(p) <- NULL
      #nomination is"yX+Mdt or Story" which signify y=ax+b
      pValTable=rbind(pValTable,list(paste("HRV",colnames(df_eeg[col1]),"Mdt"),as.numeric(p)))
